...
nextword_skip_non_letters:
    lb      $t0,                        0($a0)                                      # Load the current character
    beqz    $t0,                        nextword_end                                # If it's null, no more words
    addi    $a1,                        $a0,                    0                   # Save current pointer
    jal     isletter                                                                # Check if the character is a letter
    beq     $v0,                        $zero,                  skip_char           # If not a letter, skip
    j       nextword_find_word                                                      # Found the start of the next word

skip_char:
    addi    $a0,                        $a0,                    1                   # Move to the next character
    j       nextword_skip_non_letters

nextword_find_word:
    # Skip to the end of the current word
nextword_skip_word:
    lb      $t0,                        0($a0)                                      # Load the current character
    beqz    $t0,                        nextword_end                                # If null, no more words
    jal     isletter                                                                # Check if it's a letter
    beq     $v0,                        $zero,                  return_next_word    # If not a letter, end of word
    addi    $a0,                        $a0,                    1                   # Move to the next character
    j       nextword_skip_word

return_next_word:
    addi    $v0,                        $a0,                    0                   # Set $v0 to the start of the next word
    j       nextword_restore

nextword_end:
    li      $v0,                        0                                           # No next word, return 0
...
wordsmatch:
    ...
    # Find the start of the next word in $a0
    jal     nextword
    beqz    $v0,                        wordsmatch_end                              # If no next word, words don't match
    add     $t0,                        $v0,                    $zero               # Save the pointer to the next word

    # Find the start of the next word in $a1
    jal     nextword
    beqz    $v0,                        wordsmatch_end                              # If no next word, words don't match
    add     $t1,                        $v0,                    $zero               # Save the pointer to the next word

    # Compare the two words
wordsmatch_compare:
    lb      $t2,                        0($t0)                                      # Load the current character from the first word
    lb      $t3,                        0($t1)                                      # Load the current character from the second word
    beqz    $t2,                        wordsmatch_end                              # If the first word is null, the words match
    beqz    $t3,                        wordsmatch_end                              # If the second word is null, the words don't match
    jal     lettersmatch                                                            # Compare the two characters
    beq     $v0,                        $zero,                  wordsmatch_end      # If the characters don't match, the words don't match
    addi    $t0,                        $t0,                    1                   # Move to the next character in the first word
    addi    $t1,                        $t1,                    1                   # Move to the next character in the second word
    j       wordsmatch_compare
